      ...
      <div className="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 items-center gap-8">
        {features.map(({ Icon, title, desc }, i) => (
          <div key={i} className="flex flex-col items-center text-center p-4">
            <Icon className="w-16 h-16 mb-2 text-[#202332] dark:text-white transition-colors" />
            <h3 className="text-[22px] font-semibold text-[#212529] dark:text-white mb-2 mt-2">
              {title}
            </h3>
            <p className="text-[#8492a6] dark:text-gray-400 text-base">
              {desc}
            </p>
          </div>
        ))}
